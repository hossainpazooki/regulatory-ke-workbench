...
from backend.database_service.app.services.stores import (
    EmbeddingStore,
    EmbeddingType,
    GraphStore,
)
...
if TYPE_CHECKING:
    from backend.rule_service.app.services import Rule, RuleLoader
...
class RuleAnalyticsService:
    ...
    def cluster_rules(
        self,
        embedding_type: EmbeddingTypeEnum = EmbeddingTypeEnum.SEMANTIC,
        ...
        emb_type = EmbeddingType(embedding_type.value)
        ...
        elif algorithm == ClusterAlgorithm.DBSCAN:
    ...
    def find_conflicts(
        ...
        if conflict_types is None:
            conflict_types = [ConflictType.SEMANTIC, ConflictType.STRUCTURAL]
    ...
    def find_similar(
        self,
        rule_id: str,
        embedding_type: EmbeddingTypeEnum = EmbeddingTypeEnum.ALL,
        top_k: int = 10,
        min_score: float = 0.5,
